...
app.get('/directors/:Director', passport.authenticate('jwt', {session: false}), (req, res) => {
  Directors.findOne({Name: req.params.Director}) //Finds the director by name from the "Directors" collection in the database
    .then((director) => {
  ...
  Users.findOneAndRemove({Username: req.params.Username}) //This finds the user by username and removes them from the user group
    .then((user) =>{
      if (!user) {
        ...
        res.status(200).send(req.params.Username + ' was deleted successfully.'); //If the username was found it'll send this message
      }
    })
